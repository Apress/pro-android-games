...
static int CalcRotate(objtype *ob)
{
	...
	if (w0 == true || w1 == true){
		if (gamestatesWL6[ob->state].rotate == 2)  // 2 rotation pain frame
			return 4*(angle/(ANGLES/2));    // seperated by 3
	} else {
		if (gamestatesSOD[ob->state].rotate == 2)  // 2 rotation pain frame
			return 4*(angle/(ANGLES/2));    // seperated by 3
	}
...
//
	...
	{
		if (w0 == true || w1 == true){
			if (!(visptr->shapenum = gamestatesWL6[obj->state].shapenum))
				continue;  // no shape
		} else {
			if (!(visptr->shapenum = gamestatesSOD[obj->state].shapenum))
				continue;  // no shape
		}
		...
		|| (*(visspot+63) && !*(tilespot+63))) 
		{
			...
			if (w0 == true || w1 == true){
				if (gamestatesWL6[obj->state].rotate)
					visptr->shapenum += CalcRotate(obj);
			} else {
				if (gamestatesSOD[obj->state].rotate)
					visptr->shapenum += CalcRotate(obj);
			}
...
static const int weaponscaleWL6[NUMWEAPONS] = {SPR_KNIFEREADYWL6,SPR_PISTOLREADYWL6
	,SPR_MACHINEGUNREADYWL6,SPR_CHAINREADYWL6};

static const int weaponscaleSOD[NUMWEAPONS] = {SPR_KNIFEREADYSOD,SPR_PISTOLREADYSOD
	,SPR_MACHINEGUNREADYSOD,SPR_CHAINREADYSOD};
...
static void DrawPlayerWeapon()
{
	...
	if (w0 == true || w1 == true){
		if (gamestate.victoryflag)
		{
			if ((player->state == s_deathcam) && (get_TimeCount() & 32))
				SimpleScaleShape(viewwidth/2,SPR_DEATHCAMWL6,viewheight+1);
			return;
		}
	}
	...
	{
		if (w0 == true || w1 == true){
			shapenum = weaponscaleWL6[gamestate.weapon]+gamestate.weaponframe;
		} else {
			shapenum = weaponscaleSOD[gamestate.weapon]+gamestate.weaponframe;
		}
	...
	if (demorecord || demoplayback){
		if (w0 == true || w1 == true){
			SimpleScaleShape(viewwidth/2,SPR_DEMOWL6,viewheight+1);
		} else {
			SimpleScaleShape(viewwidth/2,SPR_DEMOSOD,viewheight+1);
		}
	}
}
...
static const unsigned int CeilingSOD[]=
{
...
static const unsigned int CeilingWL6[]=
{
...
static void ClearScreen()
{
	...
	if (w0 == true || w1 == true){
		unsigned int ceiling = CeilingWL6[gamestate.episode*10+mapon] & 0xFF;
		VL_Bar(xoffset, yoffset, viewwidth, viewheight / 2, ceiling);
	} else {
		unsigned int ceiling = CeilingSOD[gamestate.episode*10+mapon] & 0xFF;
		VL_Bar(xoffset, yoffset, viewwidth, viewheight / 2, ceiling);
	}
...
static byte *spritegfxWL6[SPR_TOTALWL6];
static byte *spritegfxSOD[SPR_TOTALSOD];
...
static void DeCompileSprite(int shapenum)
{
	...
	if (w0 == true || w1 == true){
		spritegfxWL6[shapenum] = buf;
	} else {
		spritegfxSOD[shapenum] = buf;
	}
}
...
void ScaleShape(int xcenter, int shapenum, unsigned height)
{
	...
	if (w0 == true || w1 == true){
		if (spritegfxWL6[shapenum] == NULL){
			DeCompileSprite(shapenum);
		}
	} else {
		if (spritegfxSOD[shapenum] == NULL){
			DeCompileSprite(shapenum);
		}
	}
	...
	for (; x < (64 << 16); x += scaler, p++) {
		...
		if (w0 == true || w1 == true){
			ScaleLineTrans(height >> 2, spritegfxWL6[shapenum] + ((x >> 16) << 6), p);
		} else {
			ScaleLineTrans(height >> 2, spritegfxSOD[shapenum] + ((x >> 16) << 6), p);
		}
	}	
}
...
void SimpleScaleShape(int xcenter, int shapenum, unsigned height)
{
	...
	for (; x < (64 << 16); x += scaler, p++) {
		...
		if (w0 == true || w1 == true){
			ScaleLineTrans(height, spritegfxWL6[shapenum] + ((x >> 16) << 6), p);
		} else {
			ScaleLineTrans(height, spritegfxSOD[shapenum] + ((x >> 16) << 6), p);
		}
	}
}
...
/*
jni_printf("vars: %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d\n", midangle, viewangle, FINEANGLES, ANGLES, viewx, TILEGLOBAL, xpartialdown
	, xpartialup, viewy, ypartialdown, ypartialup);
*/
