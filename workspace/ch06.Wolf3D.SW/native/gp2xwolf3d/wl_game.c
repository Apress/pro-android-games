...
void PlayDemo(int demonumber)
{
	...
	if (w0 == true){
		...
		CA_CacheGrChunk(dems[demonumber]);
		demoptr = grsegsWL1[dems[demonumber]];
	} else if (w1 == true){
		int dems[4]={T_DEMO0,T_DEMO1,T_DEMO2,T_DEMO3};
		CA_CacheGrChunk(dems[demonumber]);
		demoptr = grsegsWL6[dems[demonumber]];
	} else if (s0 == true){
		int dems[1]={T_DEMO0};
		CA_CacheGrChunk(dems[demonumber]);
		demoptr = grsegsSDM[dems[demonumber]];
	} else {
		int dems[4]={T_DEMO0,T_DEMO1,T_DEMO2,T_DEMO3};
		CA_CacheGrChunk(dems[demonumber]);
		demoptr = grsegsSOD[dems[demonumber]];
	}
...
void Died()
{
	...
	if (w0 == true || w1 == true){
		SD_PlaySoundWL6(PLAYERDEATHSNDWL6);
	} else {
		SD_PlaySoundSOD(PLAYERDEATHSNDSOD);
	}
